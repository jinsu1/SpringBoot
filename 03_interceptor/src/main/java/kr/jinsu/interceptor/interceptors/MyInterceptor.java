package kr.jinsu.interceptor.interceptors;
...
import kr.jinsu.interceptor.helpers.UtilHelper;
...
public class MyInterceptor implements HandlerInterceptor{
     ...
     * return 값으로 boolean 값을 전달하는데 false인 경우 controller를 실행 시키지 않고 요청을 종료한다.
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        //이전에 머물렀던 페이지가 존재하면?
        // -> 직전 종료시간과 이번 접속의 시작시간 과의 차이는 이전 페이지에 머문 시간을 의미
     ...
     * requset로 넘어온 데이터 가공 시 많이 사용된다.
     */
    @Override
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
    @Nullable ModelAndView modelAndView) throws Exception {
        //컨트롤러의 실행 종료 시간을 가져온다
